##This script will plot posterior distributions for cardiovascular mortality##
...
###Calculate the ratio of these two probabilities/risks (the risk ratio; rd)
...
###We now have a distribution of risk ratios under the primary and secondary prevention settings
###This is needed to for the plot of the distribution of BA's effect that we will create a little further below


###Now, we need to calculate the point estimate for the risk ratio and the 95% upper and lower bounds
#For primary prevention
primary_rd <- posterior_summary(primary_draws %>% select(rd))
#For secondary prevention
secondary_rd <- posterior_summary(secondary_draws %>% select(rd))
#The output is the risk ratio under primary and secondary prevention settings

#Finally, calculate the probability of benefit (reducing cvm) under the 1ry prevention setting
#and the probabiltiy of harm (increasing cvm) under the 2ry prevention setting
primary_post_prob <- b_model_cvm %>% hypothesis("ba < 0") %>%
...
secondary_post_prob <- b_model_cvm %>% hypothesis("ba + ba:prevention_statusSecondary > 0") %>%
  {.$hypothesis$Post.Prob} %>% #Extract the posterior probability 
  {. * 100} %>% #Convert to % for ease of interpretation
  round(., 2) %>% format(., nsmall = 2) #Round to 2 decimal places (and show trailing zeros)
...
interaction_post_prob <- b_model_cvm %>% hypothesis(hypothesis = "ba:prevention_statusSecondary > 0") %>%
  {.$hypothesis$Post.Prob} %>% #Extract the posterior probability 
  {. * 100} %>% #Convert to % for ease of interpretation
  round(., 2) %>% format(., nsmall = 2) #Round to 2 decimal places (and show trailing zeros)
...
main_title <- "Effect of Bempedoic Acid on Cardiovascular Mortality in the Primary and Secondary Prevention Settings"
...
#Plot
cvm_plot <- ggplot(data = combined_draws,
       aes(y = 1,
           x = rd*100,
...
ggsave(plot = cvm_plot,
       filename = "C:/Ahmed's Stuff/ResearchStuff/BA_Primary_v_Secondary/Figure 2.png",
       dpi = 600,
       width = 16,
       height = 9)
